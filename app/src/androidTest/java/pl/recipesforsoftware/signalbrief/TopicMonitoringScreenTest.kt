...
class TopicMonitoringScreenTest {
    ...
    @Test fun emptyStateAndCreateAffordanceRender() {
        var opened = false
    ...
    @Test fun rowOverflowTargetsSelectedTopicOnly() {
        ...
        content(TopicMonitoringUiState(topics = listOf(first, second)), { }, { renamed = it }, { deleted = it })
    ...
    @Test fun editorErrorAndSelectedDeleteTopicAreVisible() {
        val topic = MonitoredTopic("2", "Compose")
        content(TopicMonitoringUiState(editor = TopicEditor.Create(), error = TopicMonitoringUiError.DuplicateQuery))
        rule.onNodeWithText("This topic is already being monitored.").assertIsDisplayed()
        content(TopicMonitoringUiState(pendingDelete = topic))
